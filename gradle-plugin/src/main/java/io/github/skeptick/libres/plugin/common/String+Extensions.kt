...
private val TemplateParameterRegex = Regex("""\$\{([^}]*)}""")
...
internal fun String.isValidPropertyName() = matches(NameRegex)
...
internal fun String.extractTemplateParameters() =
    TemplateParameterRegex.findAll(this).toList().map { result ->
        result.groupValues[1]
    }

internal fun String.replaceTemplateParameters(formatter: (index: Int, name: String) -> String): String {
    var index = 1
    return replace(TemplateParameterRegex) { formatter(index++, it.groupValues[1]) }
}